/**
 * Edge Function: end-trip
 *
 * Marks a trip as completed, records the actual completion timestamp,
 * and signals the end of GPS tracking for that trip.
 * Called by a driver (or operator/admin) to signal the end of a trip.
 *
 * Body: { trip_id: string }
 *
...
async function logAuditEvent(
  ...
  if (error) {
    console.error("[end-trip][audit] Failed to log:", error.message);
  }
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Validate trip status is 'in_progress'
    if (trip.status !== "in_progress") {
      return new Response(
        JSON.stringify({
          error: "Trip cannot be ended: current status is not 'in_progress'",
          code: "INVALID_STATUS",
    ...
    // Update trip: set status to completed and actual_completion to now
    ...
    if (updateError || !updatedTrip) {
      return new Response(
        JSON.stringify({ error: "Failed to end trip", code: "UPDATE_FAILED", details: updateError?.message } as ErrorResponse),
        { status: 500, headers }
      );
    }

    // Log trip_completed event to audit log
    ...
    console.error("[end-trip] Unhandled error:", err);
    return new Response(
